...
from myapp.models import Course,Session_Year,CustomUser,Student,Staff,Subject,Staff_Notification,Staff_leave,Staff_Feedback,Student_Notification,Student_Feedback,Student_leave,Attendance,Attendance_Report
...
def ADD_STUDENTS(request):
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(email=email).exists():
            messages.warning(request,'Email is already there')
            return redirect('add_student')
        if CustomUser.objects.filter(username=username).exists():
            messages.warning(request,'Username is already there')
            return redirect('add_student')
        else:
            user = CustomUser(
...
def ADD_STAFF(request):
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username = username).exists():
            messages.warning(request,'Username is already existing!')
            return redirect('add_staff')
        else:
            ...
            staff.save()
            messages.success(request,'Professor is successfully added')
            return redirect('add_staff')
...
def DELETE_STAFF(request, admin):
    ...
    staff.delete()
    messages.success(request,'Record is succesfully deleted')
    return redirect('view_staff')
    return None
...
def UPDATE_SESSION(request):
    if request.method == "POST":
        ...
        session.save()
        messages.success(request,'Session year is successfully updated')
